    ...
    write_canonical_json(
        ...
        &json!({
            ...
            "definitions": [
                ...
                {"id":"performance-custody-placement", "assertion":"12 hot baseline series and 396 cold baseline copies retain distinct pre/post full hashes; initial byte equality inherits the verified master logical state; 12 hot post and 396 deferred cold post logical/schema checks reject every unexplained change before acceptance; cold post validation follows the entire alternating matrix; candidate pre/post full-slot custody remains; 816 baseline and 1584 candidate working-file hash reads, no cleanup/probe-only rehash; io-plan reports retained CoW storage and residual cache conditioning"},
