...
start_codex_watch() {
  if [ ! -d "$CODEX_WATCH_DIR" ]; then
    echo "[Fri Mar 13 20:01:19 PDT 2026] codex watch skipped: $CODEX_WATCH_DIR missing" >> "$LOG_FILE"
    return 0
  fi
  ...
  /opt/homebrew/bin/fswatch -0 -r --event Created --event Updated --event Renamed     "$CODEX_WATCH_DIR"   | while IFS= read -r -d "" changed_path; do
      case "$changed_path" in
        *.jsonl)
          echo "[Fri Mar 13 20:01:19 PDT 2026] codex change: $changed_path" >> "$LOG_FILE"
          sleep "$DEBOUNCE_SECS"
          run_ingest "$HOME" "$changed_path" "codex"
          ;;
      esac
